...
object TranscriptTailReader {
    ...
    fun lastMessages(path: Path, count: Int, chunkSize: Int = DEFAULT_CHUNK_SIZE): List<ConversationMessage> {
    ...
    private fun offer(bytes: ByteArray, from: Int, to: Int, into: MutableList<ConversationMessage>) {
        if (to <= from) return
        val line = String(bytes, from, to - from, StandardCharsets.UTF_8)
        if (line.isBlank()) return
        ConversationMessages.parseLine(line)?.let { into += it }
    }
}
