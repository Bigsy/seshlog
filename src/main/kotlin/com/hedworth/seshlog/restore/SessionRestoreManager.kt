...
class SessionRestoreManager(private val project: Project) : Disposable {
    ...
    /** Previous-run ids retained until successfully launched or explicitly dismissed. */
    @Volatile
    ...
    fun recordLaunch(session: Session) {
        ...
        pending = pending.filterNot { it == session.id }
    ...
    /** Called once from the startup activity, on the EDT. */
    fun start() {
        ...
        connection.subscribe(ProjectCloseListener.TOPIC, object : ProjectCloseListener {
            override fun projectClosingBeforeSave(project: Project) {
                ...
                if (project == this@SessionRestoreManager.project) snapshotAtClose(index.sessions)
            }
        })
        connection.subscribe(AppLifecycleListener.TOPIC, object : AppLifecycleListener {
            override fun appClosing() {
                // Application settings are saved before individual projects close. Checkpoint
                // now, but do not freeze permanently: the user can still cancel IDE exit.
                snapshots.invalidate()
                preserveAtClose(index.sessions)
            }

            override fun appWillBeClosed(isRestart: Boolean) = snapshotAtClose(index.sessions)
        })
    ...
    internal fun loadPending() {
        pending = state.liveSessionIds
    }

    internal fun applySnapshot(ids: List<String>) {
        if (!project.isDisposed && !closing) remember((ids + pending).distinct())
    }

    internal fun dismissRestore(ids: Collection<String>) {
        if (ids.isEmpty()) return
        snapshots.invalidate()
        pending = pending.filterNot { it in ids }
        remember(state.liveSessionIds.filterNot { it in ids && it !in launched })
    }
    ...
    private fun snapshot(sessions: List<Session>) {
        ...
        val widgets = owned.sessionIds.mapNotNull { id -> owned.terminalFor(id)?.let { id to it } }.toMap()
    ...
    internal fun snapshotAtClose(sessions: List<Session>) {
        closing = true
        snapshots.dispose()
        preserveAtClose(sessions)
    }

    private fun preserveAtClose(sessions: List<Session>) {
    ...
    private fun offerRestore(sessions: List<Session>) {
        val ids = pending.filterNot { it in offered }
        if (ids.isEmpty()) return
        ...
        dismissRestore(resolved.filterNot { it in restorableIds })
        if (plan.restore.isEmpty()) return
        offered += restorableIds
